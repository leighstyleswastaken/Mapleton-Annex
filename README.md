...
2. **Optional: enable Gemini-generated logs**

   The game is fully playable without an API key and automatically uses its local fallback log pools.

   To use the existing Gemini integration during local development, create a `.env` file in the root directory:
   ```env
   GEMINI_API_KEY=your_gemini_api_key_here
   ```

   Do not place a private production key into a public browser deployment. A protected server-side integration is a separate future decision.

3. **Launch the facility**
   ```bash
   npm start
   ```
   Open [http://localhost:3000](http://localhost:3000) to begin your shift.
...
The production interface is self-contained:
- Tailwind is compiled locally through PostCSS and Vite
- Courier Prime, Inter and Comic Neue are bundled from local npm packages
- React, React DOM and the existing Gemini client are bundled from npm
- `start_screen.jpg` is stored in the repository
- sound effects are generated procedurally with the Web Audio API
- Gemini is optional; local fallback logs preserve the complete playable loop when it is unavailable or unconfigured
...
## 6. Development Notes
- **AI Backend**: The existing optional integration uses Gemini 2.0 Flash to generate unique log text based on the current game state. Without a configured key, the game uses local authored fallback pools.
- **Audio**: All sound effects (chimes, hums, buzzers) are generated procedurally via the Web Audio API. No audio assets are loaded.
